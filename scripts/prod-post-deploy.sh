...
# Postgres `erp_app_tenant` avec un mot de passe en dur
# ("erp_tenant_dev_password", acceptable en dev/CI uniquement) — on ne
# modifie jamais une migration déjà appliquée (CLAUDE.md), donc ce script
# fait tourner les migrations puis remplace ce mot de passe par une vraie
# valeur secrète (TENANT_ROLE_PASSWORD). Idempotent : peut être rejoué sans
# effet de bord.
set -euo pipefail
...
echo "==> Terminé. Vérifiez que TENANT_DATABASE_URL (docker/.env.prod) utilise bien TENANT_ROLE_PASSWORD, puis démarrez l'API :"
