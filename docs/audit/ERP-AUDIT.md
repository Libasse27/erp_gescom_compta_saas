...
- **Solution** : Introduire un header `Idempotency-Key` obligatoire sur ces routes, table
  ...
  généraliser).
- **Priorité** : P1
- **Statut** : CORRIGÉ (2026-08-16) — `POST /sales`, `POST /purchases`,
  `POST /invoices` et `POST /stock/movements` corrigés (docs/adr/0019-...),
  par contrainte unique `(enterpriseId, idempotencyKey)` par modèle plutôt
  que par la table générique `(enterpriseId, key, endpoint) -> résultat`
  suggérée ici (écart assumé et justifié dans l'ADR : sur-ingénierie pour
  les endpoints connus, le patron par contrainte unique est déjà éprouvé
  dans ce dépôt sur `Payment`). `StockRepository.applyMovement`, partagé
  avec `SalesRepository.confirm()`/`PurchasesRepository.confirm()`, garde
  la clé optionnelle : ces deux appelants n'en fournissent jamais, déjà
  protégés par leur propre machine à états — seul `POST /stock/movements`
  (StockController) en fournit une, via l'en-tête. `POST /sales/:id/confirm`,
  `POST /purchases/:id/confirm`, `POST /invoices/:id/mark-paid` restent non
  traités : transitions d'état déjà protégées par leur machine à états (un
  rejeu échoue avec 409, pas de corruption silencieuse — hors du périmètre
  CRITICAL identifié par MOBILE AUDIT-001).
