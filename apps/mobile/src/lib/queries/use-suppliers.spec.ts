...
import { useDeactivateSupplier, useSaveSupplier, useSupplier, useSuppliers } from "./use-suppliers";
...
const SAMPLE_VALUES: CreateSupplierInput = {
  type: "COMPANY",
...
describe("useSuppliers", () => {
  it("construit les paramètres de requête depuis les filtres", async () => {
    ...
    await renderHook(() => useSuppliers({ page: 1, pageSize: 20, search: "sonatel", isActive: true }), { wrapper });
  ...
  it("ne lance aucune requête tant que non authentifié", async () => {
    ...
    await renderHook(() => useSuppliers({ page: 1, pageSize: 20 }), { wrapper });
  ...
  it("lève une erreur sur réponse non-ok", async () => {
    ...
    const { result } = await renderHook(() => useSuppliers({ page: 1, pageSize: 20 }), { wrapper });
...
describe("useSupplier", () => {
  it("ne lance rien sans id", async () => {
    ...
    await renderHook(() => useSupplier(undefined), { wrapper });
  ...
  it("récupère la fiche par id", async () => {
    mockAuth("authenticated", "access-1");
    mockApiFetch.mockResolvedValueOnce(response(200, { id: "s1", name: "Sonatel" }));

    const { result } = await renderHook(() => useSupplier("s1"), { wrapper });

    await waitFor(() => expect(result.current.data).toBeDefined());
    expect(mockApiFetch).toHaveBeenCalledWith("/suppliers/s1", expect.anything());
  });
});

describe("useSaveSupplier", () => {
  it("enqueue une création (POST /suppliers) puis rejoue et invalide si en ligne", async () => {
    mockAuth("authenticated", "access-1");
    ...
    const { result } = await renderHook(() => useSaveSupplier(), { wrapper });
    await act(async () => {
      await result.current({ values: SAMPLE_VALUES });
    });

    expect(mockEnqueueMutation).toHaveBeenCalledWith({ method: "POST", path: "/suppliers", body: SAMPLE_VALUES });
    expect(mockProcessQueue).toHaveBeenCalled();
  });

  it("enqueue une modification (PATCH /suppliers/:id)", async () => {
    mockAuth("authenticated", "access-1");
    mockUseIsOnline.mockReturnValue(true);
    ...
    const { result } = await renderHook(() => useSaveSupplier(), { wrapper });
    await act(async () => {
      await result.current({ supplierId: "s1", values: SAMPLE_VALUES });
    });
...
describe("useDeactivateSupplier", () => {
  it("enqueue une suppression (DELETE /suppliers/:id)", async () => {
    mockAuth("authenticated", "access-1");
    mockUseIsOnline.mockReturnValue(true);
    mockEnqueueMutation.mockResolvedValueOnce(makeQueuedMutation({ method: "DELETE" }));
    mockProcessQueue.mockResolvedValueOnce(undefined);

    const { result } = await renderHook(() => useDeactivateSupplier(), { wrapper });
    await act(async () => {
      await result.current("s1");
    });

    expect(mockEnqueueMutation).toHaveBeenCalledWith({ method: "DELETE", path: "/suppliers/s1" });
  });
  ...
  it("propage le rejet serveur plutôt que de le traiter comme un succès (revue sécurité)", async () => {
    ...
    await act(async () => {
      await expect(result.current("s1")).rejects.toThrow("HTTP 403");
    });
