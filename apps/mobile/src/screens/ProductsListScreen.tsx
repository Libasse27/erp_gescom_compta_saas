...
import { useDeactivateProduct, useProducts } from "../lib/queries/use-products";
...
export function ProductsListScreen({ navigation }: Props) {
  ...
  // Même patron que SuppliersListScreen (Phase 9.5) : signale les mutations
  // en échec terminal, sans UI de retry/dismiss par ligne (reporté,
  // docs/adr/0015-...).
  ...
  useLayoutEffect(() => {
    navigation.setOptions({
      headerRight: () => (
        <Pressable onPress={() => navigation.navigate("ProductForm", undefined)} hitSlop={8}>
  ...
  return (
    <View style={styles.container}>
      ...
      <TextInput
        ...
        placeholder="Code, nom ou code-barres…"
        value={searchInput}
      ...
      {productsQuery.isLoading ? <ActivityIndicator style={styles.loader} /> : null}
      {productsQuery.isError ? <Text style={styles.errorText}>Impossible de charger les produits.</Text> : null}
    ...
    <Pressable style={styles.row} onPress={onPress}>
      <View style={styles.rowMain}>
        ...
        <Text style={styles.rowSubtitle}>
          {formatFCFA(product.sellingPriceExcludingTax)} · TVA {product.vatRateBasisPoints / 100}%
          {product.trackStock ? " · Stock suivi" : ""}
        </Text>
      </View>
      {confirming ? (
